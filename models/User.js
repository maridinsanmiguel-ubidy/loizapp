...
User.prototype.validate = function() {
    return new Promise(async (resolve, reject) => {
        ...
        if (this.data.username != "" && !validator.isAlphanumeric(this.data.username)) {
            this.errors.push("Username only contain letters and numbers.")
        }
        ...
        if (this.data.password.length > 0 && this.data.password.length < 12) {
            this.errors.push("Password must be atlease 12 characters.")
        }
        if (this.data.password.length > 50) {
            this.errors.push("Password cannot exceed 50 characters.")
        }
        if (this.data.username.length > 0 && this.data.password.length < 3) {
            this.errors.push("Username must be atlease 3 characters.")
        }
        ...
        if (validator.isEmail(this.data.email)) {
            ...
            if (emailExist) {this.errors.push("That email is being used.")}
        }
        resolve()
    })
}
